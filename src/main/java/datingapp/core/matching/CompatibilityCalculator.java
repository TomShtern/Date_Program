...
/** Calculates compatibility scores between users using configured weights and business rules. */
public class CompatibilityCalculator {
     ...
     * <p>Do not call public scoring methods on a base instance created this way; the service
     * dependencies are intentionally uninitialized and those methods will fail fast.
     */
    protected CompatibilityCalculator() {
        this.config = null;
        this.clock = null;
    }

    public CompatibilityCalculator(AppConfig config) {
        this(config, AppClock.clock());
    }

    public CompatibilityCalculator(AppConfig config, Clock clock) {
    ...
    private void requireConfig() {
        if (config == null) {
            throw new IllegalStateException("CompatibilityCalculator config is not initialized");
        }
    }

    private void requireClock() {
        if (clock == null) {
            throw new IllegalStateException("CompatibilityCalculator clock is not initialized");
        }
    }
}
