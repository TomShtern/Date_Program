...
public class CandidateFinder implements CandidateFinderService {
     ...
     *
     * Filtering rules (ALL must be true for a candidate):
     * 1. Not self
     * 2. Not already interacted (liked/passed)
     * 3. Mutual gender preferences (both ways)
     * 4. Mutual age preferences (both ways)
     * 5. Within seeker's distance preference
     * 6. Passes seeker's dealbreakers (Phase 0.5b)
     *
     ...
     * - Seeker is interested in candidate's gender
    ...
    /**
     * Checks if age preferences match both ways:
    ...
    private boolean hasMatchingAgePreferences(User seeker, User candidate) {
        ...
        boolean candidateInSeekerRange = candidateAge >= seeker.getMinAge()
                && candidateAge <= seeker.getMaxAge();
        boolean seekerInCandidateRange = seekerAge >= candidate.getMinAge()
                && seekerAge <= candidate.getMaxAge();
