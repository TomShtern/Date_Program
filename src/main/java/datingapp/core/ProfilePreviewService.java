...
/**
 * Service for generating profile previews.
 * Shows users how their profile appears to others, with completeness
...
public class ProfilePreviewService {
    ...
    public ProfilePreview generatePreview(User user) {
        ...
        String displayLookingFor = user.getLookingFor() != null
                ? user.getLookingFor().getDisplayName()
                : null;
    ...
    public ProfileCompleteness calculateCompleteness(User user) {
        ...
        checkField("Interested In", user.getInterestedIn() != null && !user.getInterestedIn().isEmpty(), filled,
                missing);
