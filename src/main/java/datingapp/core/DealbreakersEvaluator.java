...
/**
 * Evaluates whether a candidate passes all of a seeker's dealbreakers. Pure Java - no framework
 * dependencies.
 *
...
public class DealbreakersEvaluator {
    ...
    public boolean passes(User seeker, User candidate) {
        ...
        if (db.hasKidsDealbreaker()) {
            if (candidate.getWantsKids() == null || !db.acceptableKidsStance().contains(candidate.getWantsKids())) {
                return false;
            }
        }
        ...
        if (db.hasLookingForDealbreaker()) {
            if (candidate.getLookingFor() == null || !db.acceptableLookingFor().contains(candidate.getLookingFor())) {
                return false;
            }
        }
        ...
        if (db.hasEducationDealbreaker()) {
            if (candidate.getEducation() == null || !db.acceptableEducation().contains(candidate.getEducation())) {
                return false;
            }
        }
