...
public class ReportService {
    ...
    public ReportService(ReportStorage reportStorage, UserStorage userStorage, BlockStorage blockStorage) {
        this(reportStorage, userStorage, blockStorage, AppConfig.defaults());
    }

    /**
     * File a report against a user. Returns a ReportResult indicating success/failure and if user was
     * auto-banned.
     */
    public ReportResult report(
            java.util.UUID reporterId, java.util.UUID reportedUserId, Report.Reason reason, String description) {
        ...
        if (reportCount >= config.autoBanThreshold() && reportedUser.getState() != User.State.BANNED) {
            reportedUser.ban();
