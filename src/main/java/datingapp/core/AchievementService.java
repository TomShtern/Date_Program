...
public class AchievementService {
  ...
  public List<UserAchievement> checkAndUnlock(UUID userId) {
    ...
    for (Achievement achievement : Achievement.values()) {
      if (!achievementStorage.hasAchievement(userId, achievement)
          && isEarned(userId, user, achievement)) {
  ...
  public List<AchievementProgress> getProgress(UUID userId) {
    ...
    for (Achievement achievement : Achievement.values()) {
      ...
      progress.add(
          new AchievementProgress(achievement, currentAndTarget[0], currentAndTarget[1], unlocked));
    }
  ...
  public Map<Achievement.Category, List<AchievementProgress>> getProgressByCategory(UUID userId) {
    ...
    Map<Achievement.Category, List<AchievementProgress>> grouped =
        new EnumMap<>(Achievement.Category.class);
  ...
  private int getProfileCompleteness(User user) {
    ProfilePreviewService.ProfileCompleteness completeness =
        profilePreviewService.calculateCompleteness(user);
