...
public final class UiComponents {
    ...
    public static final class TypingIndicator extends HBox {
        ...
        public TypingIndicator() {
            ...
            for (int i = 0; i < UiConstants.TYPING_DOT_COUNT; i++) {
                Circle dot = new Circle(UiConstants.TYPING_DOT_RADIUS);
                dot.setFill(Color.web("#94a3b8"));
                dot.getStyleClass().add("typing-dot");
                getChildren().add(dot);

                // Staggered bounce animation
                TranslateTransition bounce = new TranslateTransition(UiConstants.TYPING_BOUNCE_DURATION, dot);
                bounce.setFromY(0);
                bounce.setToY(-UiConstants.TYPING_BOUNCE_DISTANCE);
    ...
    public static final class ProgressRing extends StackPane {
        ...
        public void animateTo(double value) {
            Timeline animation = new Timeline(
                    ...
                    new KeyFrame(
                            UiConstants.PROGRESS_RING_ANIMATION_DURATION,
    ...
    public static final class SkeletonLoader extends Region {
        ...
        public SkeletonLoader(double width, double height) {
            ...
            shimmerAnimation = new Timeline(new KeyFrame(UiConstants.SKELETON_SHIMMER_FRAME, e -> {
                e.consume();
                animationProgress[0] += UiConstants.SKELETON_SHIMMER_STEP;
        ...
        private static void updateGradient(Rectangle skeleton, double position) {
            double startX = UiConstants.SKELETON_GRADIENT_START + position * 2;
            double endX = startX + UiConstants.SKELETON_GRADIENT_WIDTH;
