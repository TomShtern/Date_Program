...
public class MatchingHandler {
    ...
    public MatchingHandler(ServiceRegistry services, UserSession userSession, InputReader inputReader) {
    ...
    private boolean processCandidateInteraction(User candidate, User currentUser) {
        ...
        InterestMatcher.MatchResult matchResult = InterestMatcher.compare(currentUser.getInterests(),
    ...
    private void handleMatchDetailAction(String action, Match match, User otherUser, UUID otherUserId,
            User currentUser) {
        if (action.equals("u")) {
            ...
            String confirm = inputReader
                    .readLine(CliConstants.BLOCK_PREFIX + otherUser.getName() + CliConstants.CONFIRM_SUFFIX);
            if (confirm.equalsIgnoreCase("y")) {
    ...
    private void blockFromMatches(List<Match> matches, User currentUser) {
        ...
        try {
            ...
            String confirm = inputReader
                    .readLine(CliConstants.BLOCK_PREFIX + otherUser.getName() + "? This will end your match. (y/n): ");
            if (confirm.equalsIgnoreCase("y")) {
                Block block = Block.create(currentUser.getId(), otherUserId);
                blockStorage.save(block);
                match.block(currentUser.getId());
    ...
    private void showDailyPick(DailyPickService.DailyPick pick, User currentUser) {
        ...
        InterestMatcher.MatchResult matchResult = InterestMatcher.compare(
    ...
    private void checkAndDisplayNewAchievements(User currentUser) {
        ...
        if (!newAchievements.isEmpty()) {
            ...
            for (UserAchievement ua : newAchievements) {
                logger.info("  ✨ {} - {}", ua.achievement().getDisplayName(), ua.achievement().getDescription());
            }
            logger.info("");
        }
    }
