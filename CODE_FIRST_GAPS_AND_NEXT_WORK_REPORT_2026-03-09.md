...
## 2) Preferences navigation is still broken in live code

> ⚠️ **Update (2026-03-09):** This issue has been resolved. `NavigationService.ViewType.PREFERENCES` now correctly points to `"/fxml/preferences.fxml"`.

### Evidence

- `NavigationService.ViewType.PREFERENCES` points to `"/fxml/MatchPreferences.fxml"`
- actual resource present in `src/main/resources/fxml/` is `preferences.fxml`

### Why this matters

This is not theoretical debt. It is a **real broken route** in the current UI.

- `MatchesController`
- `MatchingController`
- `PreferencesController`

That means the most brittle UI code is the least directly tested.

### Recommended next action

Add controller/integration coverage before major UI expansion or refactoring:

- FXML wiring tests
- controller action-path tests
- screen-level binding sanity tests

This will make later refactors much safer.
